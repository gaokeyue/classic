...
class DenseGraph:
    ...
    def __init__(self, weight_mat: np.array, is_complete: bool = True):
        """The vertices of a graph are set to be (0, 1, 2,...,n-1).
        :param weight_mat -- A dense graph is represented by an n-by-n numpy arrary weight_mat such that
        ...
        """
        self.mat = weight_mat.copy()
        self.n = len(weight_mat)
    ...
    def adj(self, i, is_end=False):
        ...
        if self.is_complete:
            ...
            nbrs = self.mat[:, i] if is_end else self.mat[i]
            for j in nbrs:
                if not np.isnan(j):
                    yield j

    @classmethod
    def build_random_complete_graph(cls, n):
        weight_mat = np.random.random((n, n))
        weight_mat[range(n), range(n)] = np.nan
        return cls(weight_mat, is_complete=True)
